...
## Status: ✅ Completo - Backend + UI Completa com Correções Aplicadas
...
- [x] **Task 1: Estender entidade Order**
  - [x] Subtask 1.1: Adicionar método `static getMetrics(period: 'today' | 'last7days')` que calcula métricas agregadas
  - [x] Subtask 1.2: Criar tipo `MetricsSummary` com campos: totalOrders, totalRevenue, averageOrdersPerDay (opcional)
  - [x] Subtask 1.3: Implementar cálculo de período: "today" = pedidos de hoje em UTC (00:00 UTC até agora UTC), "last7days" = últimos 7 dias incluindo hoje em UTC (7 dias atrás UTC até agora UTC) (AC 2.8.1, 2.8.21)
  - [x] Subtask 1.4: Implementar timeout (30s), logs estruturados (`admin-metrics`), mensagens de erro
  - [x] Subtask 1.5: Adicionar método `static getTopProducts(period: 'today' | 'last7days', limit: number)` que agrega por `product_id`
  - [x] Subtask 1.6: Criar tipo `TopProduct` com campos: productId, productName, totalQuantity, totalRevenue, position
  - [x] Subtask 1.7: Implementar agregação de `order_items` por `product_id` usando SQL GROUP BY com SUM de `quantity` e SUM de `total_price`, filtrando apenas produtos ativos (`status = 'Ativo'`) (AC 2.8.3, 2.8.20, 2.8.22)
  - [x] Subtask 1.8: Implementar ordenação por quantidade vendida (DESC), depois por receita total (DESC), depois por nome do produto (alfabética) em caso de empate completo (AC 2.8.3, 2.8.13)
  - [x] Subtask 1.9: Validar parâmetro `limit` (mínimo 1, máximo 50, padrão 10) (AC 2.8.23)

- [x] **Task 2: Criar API Routes protegidas**
  - [x] Subtask 2.1: `GET /api/admin/metrics?period={today|last7days}` para buscar métricas gerais
  - [x] Subtask 2.2: `GET /api/admin/metrics/top-products?period={today|last7days}&limit={10}` para buscar produtos mais vendidos
  - [x] Subtask 2.3: Adicionar verificação de role admin em todas as rotas
  - [x] Subtask 2.4: Validar parâmetros de período (apenas 'today' ou 'last7days') e limit (1-50, padrão 10) (AC 2.8.23)
  - [x] Subtask 2.5: Tratamento de erros (RLS, network, timeout, validação)
  - [x] Subtask 2.6: Logs estruturados de sucesso/falha com contexto completo (period, limit, adminId)

- [x] **Task 3: UI do Dashboard (`/admin`)**
  - [x] Subtask 3.1: Criar página `app/admin/(protected)/dashboard/page.tsx` (Dashboard principal)
  - [x] Subtask 3.2: Implementar seletor de período com tabs/botões: "Hoje" e "Últimos 7 dias"
  - [x] Subtask 3.3: Implementar badge de período no topo ("Métricas de Hoje" ou "Métricas dos Últimos 7 Dias")
  - [x] Subtask 3.4: Implementar cards de métricas:
  ...
  - [x] Subtask 3.5: Implementar seção "Produtos Mais Vendidos" com lista dos top 10
  - [x] Subtask 3.6: Cada item da lista: Posição (badge numérico), Nome, Quantidade Vendida, Receita (formatado)
  - [x] Subtask 3.7: Estados de loading independentes (skeleton para cards e lista separadamente) (AC 2.8.27)
  - [x] Subtask 3.8: Estado vazio quando não há pedidos no período e estado vazio específico para produtos (AC 2.8.25)
  - [x] Subtask 3.9: Integração com `Toast` para feedback de erros
  - [x] Subtask 3.10: Refetch automático ao alterar período (corrigido loop infinito usando `useRef`)
  - [x] Subtask 3.11: Implementar acessibilidade do dashboard (teclado, screen readers) (AC 2.8.26)

- [x] **Task 4: Estilização e UX**
  - [x] Subtask 4.1: Cards de métricas com cores distintas e ícones visuais (lucide-react)
  - [x] Subtask 4.2: Layout responsivo (grid de cards em desktop, coluna única em mobile)
  - [x] Subtask 4.3: Lista de produtos com hover effects e separadores visuais
  - [x] Subtask 4.4: Badge de posição com estilo destacado (ex: #1 em dourado, #2 em prata, #3 em bronze, outros em cinza)
  - [x] Subtask 4.5: Formatação de valores monetários com `Intl.NumberFormat` (R$ X.XXX,XX)
  - [x] Subtask 4.6: Formatação de números grandes (ex: 1.234 pedidos)
  - [x] Subtask 4.7: Formatar média diária com 1 casa decimal (AC 2.8.24)
...
- Classe `Order` estendida com métodos `getMetrics()` e `getTopProducts()`
- Tipo `MetricsSummary` criado com todos os campos necessários
- Tipo `TopProduct` criado com campos: productId, productName, totalQuantity, totalRevenue, position
- Cálculo de período em UTC implementado corretamente
- Filtro de produtos ativos usando `status = 'Ativo'` (não `deleted_at`, pois tabela `products` não possui essa coluna)
- Agregação de métricas funcionando
- Agregação de produtos mais vendidos em memória (não SQL GROUP BY devido a limitações do Supabase)
...
- Logs estruturados com prefixo `admin-metrics` em formato JSON

✅ **API Routes Implementadas:**
- `GET /api/admin/metrics?period={today|last7days}` - Buscar métricas gerais
- `GET /api/admin/metrics/top-products?period={today|last7days}&limit={10}` - Buscar produtos mais vendidos
...
- Logs estruturados JSON com campos: period, limit, productsCount, adminId, timestamp, success, error

✅ **UI Implementada:**
- Página `/admin/dashboard` com dashboard completo
- Seletor de período com tabs "Hoje" e "Últimos 7 dias"
- Badge de período no topo ("Métricas de Hoje" ou "Métricas dos Últimos 7 Dias")
- Cards de métricas: Total de Pedidos, Receita Total, Média Diária (condicional)
- Ícones visuais (ShoppingBag, DollarSign, TrendingUp)
- Cores distintas para cada card (azul, verde, roxo)
- Seção "Produtos Mais Vendidos" com lista dos top 10
- Badge de posição com cores (#1 dourado, #2 prata, #3 bronze)
- Loading independente para métricas e produtos
- Estados de loading (skeleton), vazio (sem pedidos), e "Nenhum produto vendido"
- Formatação correta de valores monetários e números
- Formatação de média diária com 1 casa decimal
- Acessibilidade: ARIA labels, navegação por teclado
- Responsivo (grid de cards em desktop, coluna única em mobile)

✅ **Infraestrutura:**
- RLS já configurado (Stories 2.6, 2.7) permite admins lerem todos os pedidos
- Índices existentes em `created_at` e `product_id` são utilizados
- Métricas calculadas em tempo real (sem cache)

✅ **Correções Aplicadas:**
- **Correção 1:** Filtro de produtos ativos alterado de `deleted_at IS NULL` para `status = 'Ativo'` (tabela `products` não possui coluna `deleted_at`, usa campo `status`)
- **Correção 2:** Loop infinito de requisições corrigido usando `useRef` para estabilizar `toast` e removendo dependências desnecessárias dos `useCallback`
- **Correção 3:** Story atualizada para refletir uso de `status = 'Ativo'` ao invés de `deleted_at`

**Desvios da Story:**
- Agregação feita em memória após buscar `order_items`, não via SQL GROUP BY diretamente (Supabase não suporta GROUP BY completo nas queries). A agregação é eficiente pois agrupa por `product_id` e calcula métricas em memória.
- Rota do dashboard é `/admin/dashboard` ao invés de `/admin` diretamente (funcionalmente equivalente com redirect).

**Notas Técnicas:**
- O filtro de produtos ativos usa `status = 'Ativo'` pois a tabela `products` não possui soft delete (`deleted_at`), apenas campo `status` ('Ativo' | 'Inativo')
- O uso de `useRef` para `toast` evita recriações desnecessárias das funções `loadMetrics` e `loadTopProducts`
- A agregação em memória é eficiente para até 50 produtos (limite máximo)
- O loading independente melhora UX - usuário vê dados assim que disponíveis
...
| 2024-11-09 | v2.0 | Implementação completa: Backend (Order.getMetrics e getTopProducts), API routes (GET /api/admin/metrics e /api/admin/metrics/top-products), UI (página /admin/dashboard com cards de métricas, seletor de período, lista de produtos mais vendidos), loading independente, acessibilidade completa. Build passando sem erros. | Dev Agent |
| 2024-11-09 | v2.1 | Correções pós-implementação: Corrigido filtro de produtos ativos (alterado de `deleted_at IS NULL` para `status = 'Ativo'` - tabela `products` não possui coluna `deleted_at`). Corrigido loop infinito de requisições usando `useRef` para estabilizar `toast` e removendo dependências desnecessárias dos `useCallback`. Story atualizada para refletir uso de `status = 'Ativo'` ao invés de `deleted_at` em ACs e Dev Notes. | Dev Agent |
