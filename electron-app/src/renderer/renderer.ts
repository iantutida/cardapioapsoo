...
class OrdersApp {
  ...
  private async init() {
    ...
    // Expor métodos globais para callbacks
    window.ordersApp = {
  ...
  private async loadAllOrders() {
    ...
    try {
      // Carregar pedidos de cada status em paralelo
  ...
  private setupRealtimeSubscriptions() {
    ...
    // Subscription para INSERT (novos pedidos)
    try {
      ...
      // Subscription para UPDATE (mudanças de status)
  ...
  private handleNewOrder(order: Order) {
    ...
    if (this.notifiedOrderIds.has(order.id)) {
      ...
      // Mesmo assim, adicionar à coluna se não estiver lá
    ...
    // Adicionar à coluna Recebidos
    this.columns[0].orders.unshift(order)
    this.renderKanban()

    // Exibir notificação visual e sonora
  ...
  private handleStatusUpdate(newOrder: Order, oldOrder: Order) {
    ...
    // Filtrar pedidos das últimas 24 horas
    ...
    // Verificar se há atualização otimista em progresso
    ...
    if (optimisticUpdate) {
      ...
      // Se a atualização Realtime é mais recente que a otimista, há conflito
      if (realtimeTime > optimisticTime && newOrder.status !== optimisticUpdate.status) {
        ...
        // Confirmação da atualização otimista
        this.optimisticUpdates.delete(newOrder.id)
      }
    }

    // Mover pedido para coluna correta
  ...
  private moveOrderToColumn(orderId: string, newStatus: 'Recebido' | 'Em Preparo' | 'Pronto', orderData?: Order) {
    ...
    if (targetColumn && orderData) {
      ...
      // Ordenar por created_at desc, depois total desc
  ...
  private renderKanban() {
    ...
    // Adicionar event listeners para drag-and-drop e botões
    this.columns.forEach(col => {
  ...
  private setupColumnDragAndDrop(columnElement: HTMLElement, column: KanbanColumn) {
    ...
    columnElement.addEventListener('dragover', (e) => {
      ...
      // Encontrar pedido sendo arrastado
  ...
  private setupCardClickHandlers(cardElement: HTMLElement, order: Order) {
    ...
    // Clique no card para ver detalhes (mas não quando arrastando)
    cardElement.addEventListener('click', (e) => {
      // Não abrir modal se clicou no botão de ação
      if ((e.target as HTMLElement).closest('.order-card-kanban-button')) {
        return
      }
      // Não abrir modal se está arrastando
